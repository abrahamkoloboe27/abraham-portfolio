/**
 * Brand marks maintained in-repo.
 *
 * lucide-react removed every brand icon in v1. These are the only four the site
 * needs, inlined as paths so upgrading the icon library never breaks the social
 * links again — and so no extra dependency is pulled in for four glyphs.
 */

import type { ComponentPropsWithoutRef } from "react";

type IconProps = Omit<ComponentPropsWithoutRef<"svg">, "children" | "viewBox" | "fill">;

function Mark({ path, label, ...props }: IconProps & { path: string; label: string }) {
  // Decorative next to a text label, announced when standing alone.
...
const PATHS = {
  ...
  x:
    ...
    "1.154h7.594l5.243 6.932ZM17.61 20.644h2.039L6.486 3.24H4.298L17.61 20.644Z",
} as const;

export function GithubIcon(props: IconProps) {
  return <Mark path={PATHS.github} label="GitHub" {...props} />;
}

export function LinkedinIcon(props: IconProps) {
  return <Mark path={PATHS.linkedin} label="LinkedIn" {...props} />;
}

export function YoutubeIcon(props: IconProps) {
  return <Mark path={PATHS.youtube} label="YouTube" {...props} />;
}

export function XIcon(props: IconProps) {
  return <Mark path={PATHS.x} label="X" {...props} />;
}
